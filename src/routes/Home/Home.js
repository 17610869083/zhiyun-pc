...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    request(api_newest_warning_opinion).then((res) => {
      ...
      this.setState({newestKeys: keys, newestValues: values});
      // 昨日和今日舆情
      request(api_today_opinion).then((res) => {
        const {yesterdayOpinion, todayOpinion} = formatTodayOpinion(res.data);
        this.setState({yesterdayOpinion: yesterdayOpinion, todayOpinion: todayOpinion});
        // 热搜媒体排行
        request(api_media_opinion).then((res) => {
          const {mediaChartOption} = formatMediaChart(res.data);
          this.setState({mediaChartOption: mediaChartOption});
          request(api_media_count).then((res) => {
            const posNegCount = formatPosNegCount(res.data.series[0].data);
            this.setState({posNegCount: posNegCount});
            // 最新舆情
            request(api_newest_opinion).then((res) => {
              ...
              request(api_newest_negative_opinion).then((res) => {
                const {keys, values} = formatOpinion(res.data);
                this.setState({newestNegativeKeys: keys, newestNegativeValues: values});
                // 微博舆情
                request(api_weibo_opinion).then((res) => {
                  const {keys, values} = formatWeiboOpinion(res.data);
                  this.setState({weiboOpinionKeys: keys, weiboOpinionValues: values});
                  // 人物舆情
                  request(api_people_opinion).then((res) => {
                    const {keys, values} = formatOpinion(res.data);
                    this.setState({peopleOpinionKeys: keys, peopleOpinionValues: values});
                    // 舆情统计
  ...
  render() {
    return (<div className="home">
      <Row gutter={16} className="row">
        <Col span={12}>
          <OpinionBox newestKeys={this.state.newestKeys} newestValues={this.state.newestValues} header="newestOpinion"/>
        </Col>
        <Col span={12}>
      ...
      <Row gutter={16} className="row">
        ...
        <Col span={12}>
          <OpinionBox newestKeys={this.state.newestNegativeKeys} newestValues={this.state.newestNegativeValues} header="newestNegativeOpinion"/>
        </Col>
      </Row>
      <Row gutter={16} className="row">
        <Col span={12}>
          <OpinionBox newestKeys={this.state.peopleOpinionKeys} newestValues={this.state.peopleOpinionValues} header="peopleOpinion"/>
        </Col>
        <Col span={12}>
          <OpinionBox newestKeys={this.state.weiboOpinionKeys} newestValues={this.state.weiboOpinionValues} header="weiboOpinion"/>
        </Col>
      </Row>
      <Row gutter={16} className="row">
        <Col span={12}>
          <TopicOpinionBox data={this.state.topicOpinionData} title="专题舆情"/>
        </Col>
        <Col span={12}>
