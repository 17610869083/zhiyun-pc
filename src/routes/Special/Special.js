...
class Special extends React.Component{
	...
	hanldle= (data,topicid) => {
        ...
        this.setState({
				...
				this.setState({
					docList:data.data[item].informationExcerpt,
					modalId:item
				})
			}
			return ''
      ...
      //   x:'center'
		...
		return (
			<div className="col">
															  ...
															  <span>六. 摘录</span>
																{
																	Object.keys(this.state.jiaData.data).map(item => (
																		this.state.componentId[6] === item ? (
																			(() => {
                                        if (this.state.jiaData.data[item].edit === "1") {
																					return <Button key={item} style={{ display: "inline-block", float: "right" }}
				...
				<Modal visible = {this.state.visible} footer={null} width="70%" 
				className="report-modal" onCancel={this.cancel}
				> 
				  <ModalReport
